...
NUM_OF_SCANNING_MACHINE = 4
NUM_OF_SCANNING_MACHINE_PRE = 2
    ...
    def customer(self, env, name, counter1, counter2, counter3,
        ...
        with counter1.request() as req:
            ...
            if req in results:
                ...
                queue_length[2][self.id] = len(counter2[1].queue)
                queue_length[3][self.id] = len(counter2[2].queue)
                queue_length[4][self.id] = len(counter2[2].queue)
        ...
        if self.pre_check:
            Qlength = [NoInSystem(counter2[i]) for i in range(2)]
        ...
        queue_length[5][self.id] = len(counter3[0].queue)
        queue_length[6][self.id] = len(counter3[1].queue)
        queue_length[7][self.id] = len(counter3[2].queue)
        queue_length[8][self.id] = len(counter3[2].queue)
...
class SecurityCheck:
    def __init__(self,new_customers,time_in_zone_A_reg, time_in_zone_B_reg, time_in_zone_C_reg,
        ...
        self.queue_length = [[0] * new_customers, [0] * new_customers, [0] * new_customers, [0] * new_customers,
                             [0] * new_customers, [0] * new_customers,
                             [0] * new_customers, [0] * new_customers, [0] * new_customers]
...
counter1_reg = simpy.Resource(env, capacity=4)

counter2_reg = [simpy.Resource(env, capacity=1), simpy.Resource(env, capacity=1), simpy.Resource(env, capacity=1), simpy.Resource(env, capacity=1)]
counter3_reg = [simpy.Resource(env, capacity=1), simpy.Resource(env, capacity=1), simpy.Resource(env, capacity=1), simpy.Resource(env, capacity=1)]
counter1_pre = simpy.Resource(env, capacity=3)
...
INTERVAL_CUSTOMERS = 3600.0/1327 #1.0/0.3687  # Generate new customers roughly every x seconds
...
dataframe1 = pandas.DataFrame(numpy.array(regular.time_stamp + [waiting_time] + regular.queueing_time + regular.service_time + regular.queue_length + [regular.choice_machine] + [regular.pre_check]).T,
                              columns=['arrivalA','arrivalB','arrivalC','leaveC','waiting time','QtimeA','QtimeB','QtimeC','serviceTA','serviceTB','serviceTC','Qlen_A','QlenB0','QlenB1','QlenB2','QlenB3','QlenC0','QlenC1','QlenC2','QlenC3','choice','precheck'])
dataframe1.to_csv("1300C2lanes45_modify3.1.csv")
