...
class customer_ob:
    def __init__(self, id):
        ...
        self.choice = 0
        self.machine_doc_check = random.random() > 0.4
    ...
    def customer(self, env, name, counter1, counter2, counter3,
        ...
        queue_length[0][self.id] = len(counter1[0].queue)
        queue_length[1][self.id] = len(counter1[1].queue)
        ...
        if self.pre_check:
            ...
            if self.machine_doc_check:
                m = 1
            else:
                m = 0
        with counter1[m].request() as req:
            ...
            if req in results:
                ...
                queue_length[2][self.id] = len(counter2[0].queue)
                queue_length[3][self.id] = len(counter2[1].queue)
...
class SecurityCheck:
    def __init__(self,new_customers,time_in_zone_A_reg, time_in_zone_B_reg, time_in_zone_C_reg,
        ...
        self.queue_length = [[0] * new_customers, [0] * new_customers,
        ...
        for i in range(number):
            ...
            if customeri.pre_check:
                c = customeri.customer(env, 'Customer%02d' % i, counter1_pre, counter2_pre, counter3_pre,
                                       ...
                                       self.queueing_time, self.service_time, self.time_stamp, self.queue_length,
                                       self.choice_machine)
            else:
...
counter1_reg = [simpy.Resource(env, capacity=2), simpy.Resource(env, capacity=3)]
...
counter1_pre = [simpy.Resource(env, capacity=2), simpy.Resource(env, capacity=1)]
...
NEW_CUSTOMERS = 1327  # Total number of customers
...
dataframe1 = pandas.DataFrame(numpy.array(regular.time_stamp + [waiting_time] + regular.queueing_time + regular.service_time + regular.queue_length + [regular.choice_machine] + [regular.pre_check]).T,
                              columns=['arrivalA','arrivalB','arrivalC','leaveC','waiting time','QtimeA','QtimeB','QtimeC','serviceTA','serviceTB','serviceTC','Qlen_A0','Qlen_A1','QlenB0','QlenB1','QlenB2','QlenC0','QlenC1','QlenC2','choice','precheck'])
dataframe1.to_csv("modify2_det.csv")
