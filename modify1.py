...
INTERVAL_CUSTOMERS = 1.0/0.3687  # Generate new customers roughly every x seconds
...
def machine_ava(machine_avail):
    if equal_ele(machine_avail) and machine_avail[0] == True:
        choose = int(random.random() * len(machine_avail))
        machine_avail[choose] = False
        return choose
    for i in range(len(machine_avail)):
        if machine_avail[i]:
            machine_avail[i] = False
            return i
...
class customer_ob:
    ...
    def customer(self, env, name, counter1, counter2, counter3,
                 ...
                 queueing_time, service_time, time_stamp, queue_length,
                 choice_machine, machine_avail):
        ...
        with counter1.request() as req:
            ...
            if req in results:
                ...
                if self.pre_check:
                    ...
                    yield env.process(self.check_queue_queue(counter2,135))
        ...
        with counter2[0].request() as req:
            patience = random.uniform(MIN_PATIENCE, MAX_PATIENCE)
            # Wait for the counter or abort at the end of our tether
            results = yield req | env.timeout(patience)
            ...
            if not self.pre_check:
                self.choice = machine_ava(machine_avail)
            if req in results:
                # We got to the counter
                ...
                if not self.pre_check:
                    machine_avail[self.choice] = True
            else:
                # We reneged
                print('%7.4f %s: RENEGED after %6.3f' % (env.now, name, wait))
        ...
        if self.pre_check:
            ...
            k = self.choice
        with counter3[k].request() as req:
...
class SecurityCheck:
    def __init__(self,new_customers,time_in_zone_A_reg, time_in_zone_B_reg, time_in_zone_C_reg,
        ...
        self.machine_avail = [True, True, True]
        ...
        for i in range(number):
            ...
            if customeri.pre_check:
                c = customeri.customer(env, 'Customer%02d' % i, counter1_pre, counter2_pre, counter3_pre,
                                       ...
                                       self.queueing_time, self.service_time, self.time_stamp, self.queue_length, self.choice_machine,self.machine_avail)
            else:
...
counter2_reg = [simpy.Resource(env, capacity=3), simpy.Resource(env, capacity=1), simpy.Resource(env, capacity=1)]
...
# Export to csv file
dataframe1 = pandas.DataFrame(numpy.array(regular.time_stamp + regular.queueing_time + regular.service_time + regular.queue_length + [regular.choice_machine] + [regular.pre_check]).T,
                              columns=['arrivalA','arrivalB','arrivalC','leaveC','QtimeA','QtimeB','QtimeC','serviceTA','serviceTB','serviceTC','Qlen_A','QlenB0','QlenB1','QlenB2','QlenC0','QlenC1','QlenC2','choice','precheck'])
dataframe1.to_csv("modify11.csv")
