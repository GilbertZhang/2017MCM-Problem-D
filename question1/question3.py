    ...
    def customer(self, env, name, counter1, counter2, counter3,
        ...
        with counter2[self.choice].request() as req:
            ...
            if req in results:
                ...
                if self.pre_check:
                    tib = 0.3 * max(1.3*time_in_zone_B[0] * numpy.random.weibull(time_in_zone_B[1],1) - time3,0)
                else:
                    tib = max(1.3*time_in_zone_B[0] * numpy.random.weibull(time_in_zone_B[1],1) - time3,0)
...
dataframe1.to_csv("longerPreperation.csv")
